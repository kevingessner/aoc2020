...
fn find_monsters(sq: usize, inner: &str) -> usize {
    ...
    let spacer = std::iter::repeat(".").take(sq * 8 - 20).collect::<String>();
    let re = Regex::new(&monster.join(&spacer).replace(" ", ".")).unwrap();
    println!("{}", re);
    let inner_s = &inner.replace("\n", "");
    let mc = re.captures_iter(&inner_s).count();
    let newm = monster.join(&spacer).replace("#", "O").to_string();
    if mc > 0 {
        let replaced = re.replace_all(&inner_s, |_: &regex::Captures| &newm)
            .to_owned()
            .chars()
            .collect::<Vec<_>>()
            .chunks(sq * 8)
    ...
    for t in tiles.values() {
        ...
        if sols.len() > 0 {
            for sol in sols {
                ...
                println!("{}\n", inner_string);
                for _ in 0..4 {
                    let mc = find_monsters(sq, &inner_string);
